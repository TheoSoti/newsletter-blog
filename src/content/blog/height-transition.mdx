---
title: 'How to smoothly transition an element’s height in CSS'
description: 'Learn five practical ways to smoothly transition an element’s height in CSS, from traditional techniques like max-height to modern methods with flexbox and grid. Discover upcoming features like calc-size() for even more flexibility.'
pubDate: 'Oct 22 2024'
...
In the past, we had to rely on clever hacks or turn to JavaScript to transition height. But now, there are some really cool, native solutions (and even more exciting ones just around the corner).
...
.dropdown {
	overflow: hidden;
	height: 210px; /* exemple 1 */
	height: auto; /* exemple 2 */
