...
Try playing with these examples below by tabbing or clicking and see the diferences:
...
For example, if you tab through this site, you will see a black or white (depending of the darkmode) outline on links and buttons.

But if you try to click on any buttons or links, it wont show anything.
...
- Use `:focus-visible` for a cleaner look, where focus only appears when helpful for the user. In general for all other types of elements (links, buttons ,…).
...
Now we reviewed the 2 basic :focus states, let’s dive into the advanced state `:focus-within`.
...
This will trigger for each interaction of the user on the selected area (click, tab, or other navigation methods).
