...
import AlphaSlider from '../components/RelativeColors/AlphaSlider.astro';
...
import StaticSaturation from '../components/RelativeColors/StaticSaturation.astro';
import Palette from '../components/RelativeColors/Palette.astro';
import StaticStates from '../components/RelativeColors/StaticStates.astro';
import StaticCallout from '../components/RelativeColors/StaticCallout.astro';
import StaticContrast from '../components/RelativeColors/StaticContrast.astro';
import RelativeGradient from '../components/RelativeColors/Gradient.astro';
...
```css
/* Shift hue by 30deg */
--accent: hsl(from var(--brand) calc(h + 30deg) s l);
...
Lightness is not the only channel worth adjusting. Saturation is the other half of a good palette. A base color that is vibrant at full saturation can be too intense for backgrounds and subtle UI pieces.

Relative colors let you lower saturation without losing the hue. That makes soft backgrounds and muted text that still belong to the same family.

```css
:root {
	--brand: #4b6bff;
	--brand-muted: hsl(from var(--brand) h calc(s - 35%) calc(l + 12%));
	--brand-ink: hsl(from var(--brand) h calc(s - 40%) calc(l - 20%));
}
```

The muted token is great for surfaces, and the ink token works for strong text or icons. This avoids the common trap of mixing a colorful palette with neutral grays that feel disconnected.

<Wrapper title="Saturation shifts">
	<StaticSaturation />
</Wrapper>
...
:root {
	--brand: #4b6bff;
	--strong-color: oklch(from var(--brand) calc(l - var(--strength)) c h);
	--soft-color: oklch(from var(--brand) calc(l + var(--strength) + 10%) calc(c - 18%) h);
	--accent-color: oklch(from var(--brand) l c calc(h + 20deg));
